...
for (const mode of ["admin", "event-admin"]) assert.equal(context.canOpenManagementMode(mode), false);
...
admin = true;
for (const mode of ["admin", "event-admin", "worlds"]) assert.equal(context.canOpenManagementMode(mode, { worldEditor: true }), true);
...
const views = { questViews: [view()], adminView: view(), feedbackView: view(), eventAdminView: view(), heroGrowthView: view(), settingsView: view(), worldsView: view(), kidsView: view(), guardianView: view() };
