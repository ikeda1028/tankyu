...
次はGoogle Apps ScriptをWeb API化し、アプリのイベント登録・会員情報・フィードバックをこのSheetへ読み書きする構成に移行できます。
...
- `Drive同期`: 会員情報、登録イベント、ワクワク、探究記録、フィードバックを一括送信
- イベント登録、会員情報保存、探究記録、フィードバック保存時の自動送信
...
- Google Maps本番化: イベント登録画面で住所検索、開催日時、参加申込、Map IDを使ったデザイン調整を追加する
