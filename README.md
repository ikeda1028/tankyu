...
AIアバターはローカルのIndexedDBでは `avatars` ストアに保存されます。Firebase同期時は画像本体をFirebase Storageの `avatars/{ユーザーID}/avatar.jpg` に保存し、Firestoreには `avatar` フィールドとしてシンボル、色、オーラ、プロンプト、Storageパス、表示URL、進化段階を保存します。
